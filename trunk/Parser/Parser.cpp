...
bool Update(vector<Token>& input){
	...
	Token comma('h', ",");
	Token eq('o', "=");
	bool ret = Identifier(input) && Match(input, set)&& Identifier(input) && Match(inut, eq) && Identifier(input);
	while(ret&&Match(input, comma)){
		ret = ret&& Identifier(input) && Match(inut, eq) && Identifier(input);
	}
